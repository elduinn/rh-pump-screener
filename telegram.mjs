 ...
 */
export async function sendTelegram(cfg, text, { fence = false, parseMode, chatIds: override, blocks } = {}) {
  const t = cfg.telegram ?? {};
...
/**
 * Long-poll for incoming messages and hand each to `onMessage`.
 * @param {(msg: {chatId: string, userId: number, text: string, name: string}) => Promise<void>} onMessage
 */
export async function listenTelegram(cfg, onMessage, { pollSeconds = 30 } = {}) {
