...
const token = (over = {}) => ({ address: "0x1111111111111111111111111111111111111111", symbol: "NEW", name: "New", creation_timestamp: now / 1000 - 45 * 60, volume: 300_001, market_cap: 1_000_000, liquidity: 100_000, price_change_percent5m: 2, ...over });
...
assert.equal(screenNewTokenVolume([token({ name: "ABC • Robinhood Token" })], cfg, now).length, 0);
...
assert.equal(store.fresh(rows, "robinhood", "chat-a", now).length, 1);
store.mark(rows, "robinhood", "chat-a", now);
assert.equal(store.fresh(rows, "robinhood", "chat-a", now + 29 * 60_000).length, 0);
assert.equal(store.fresh(rows, "robinhood", "chat-b", now + 29 * 60_000).length, 1);
assert.equal(new AlertCooldownStore(file, 30).fresh(rows, "robinhood", "chat-a", now + 31 * 60_000).length, 1);
