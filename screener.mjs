#!/usr/bin/env node
/**
 * Robinhood Chain — memecoin pump screener with 5-minute volume acceleration.
 * Built and VERIFIED against gmgn-cli v1.5.7 with live calls on 2026-08-18.
 *
 ...
 *   liquidity-provision surface — verified by grepping the whole package for
 *   add_liquidity / remove_liquidity / mint_position / pool_create: zero hits.
 *   `token pool` is READ-ONLY pool info. See the note at the bottom of the file.
 *
...
import { sendTelegram, verifyBot, resolveTelegramCreds, listenTelegram, sendTyping, escapeHtml } from "./telegram.mjs";
 ...
 *
 * Robinhood Chain lists real-world equities in the same trending feed. On a
 * market-cap-ordered sample of 100 the two markers below agreed perfectly:
 *   - name carries the "• Robinhood Token" suffix  -> 8 tokens
 *   - launchpad_platform === "pool_robinhood_stock_amm" -> 6 tokens, a strict
 *     subset (SPY and SPCX sit on pool_uniswap_v4 instead)
 * so the union is exactly the 8 equities and no memecoin is caught.
 *
 * Rejected alternatives, and why:
 *   - creator === "" / creation_timestamp === 0 matched 11 rows, i.e. the 8
 *     equities PLUS 3 genuine memecoins (Index, Hoodrat, Lady Marian). Using
 *     it would have silently thrown away real memecoins.
 *   - exchange === "0x8366..." looked promising on a 5-row sample but is a
 *     shared router: 35 of 100 memecoins trade through it too.
 */
const RWA_NAME_RE = /•\s*Robinhood Token/i;
function isTokenizedStock(t) {
  return RWA_NAME_RE.test(t.name ?? "") || t.launchpad_platform === "pool_robinhood_stock_amm";
}
...
function screen(rank, cfg) {
  const rows = [];
  const dropped = { noData: 0, notPumping: 0, risky: 0, stocks: 0 };
...
async function enrichShortVolume(rows, cfg) {
  ...
  for (const r of needsKline) {
    if (!/^0x[a-fA-F0-9]{40}$/.test(r.address)) continue; // Robinhood is EVM
    try {
...
// Telegram copies to the clipboard on tap.
function addressBlock(rows) {
  return {
    ...
    lines: [
      ...
      ...rows.map((row) =>
        `<b>${escapeHtml(String(row.symbol).replace(/[\r\n]/g, ""))}</b> · <code>${escapeHtml(row.address)}</code>`),
    ],
  };
}
...
async function deliverTelegram(rows, dropped, cfg, { useCooldown = false } = {}) {
  ...
  if (!cooling) {
    ...
    try {
      const res = await sendTelegram(cfg, null, { blocks: buildTelegramBlocks(rows, dropped, cfg) });
      if (res.skipped) {
  ...
  for (;;) {
    ...
    try {
      ...
      for (const chatId of chatIds) {
        ...
        const res = await sendTelegram(cfg, null, { blocks: buildNewTokenAlertBlocks(fresh, cfg), chatIds: [chatId] });
...
function listParams() {
  ...
  console.log(`\n  "trending.filters" (array) — EVM chains incl. robinhood:`);
  console.log(`    ${VALID_FILTER_TAGS.evm.join(" / ")}`);
  console.log(`  "trending.filters" (array) — sol:`);
...
async function listenLoop(cfg) {
  ...
  await listenTelegram(cfg, async ({ chatId, text, name }) => {
    ...
    // A message that is just an EVM address runs the configured
    // promptTemplate. Anything else is passed through as a raw prompt.
    const addr = body.match(/^0x[a-fA-F0-9]{40}$/) ? body : null;
    const prompt = addr
      ? (cfg.ai.promptTemplate ?? "").replaceAll("{address}", addr)
      : body;

    const stamp = new Date().toISOString().slice(11, 19);
    console.log(`  [listen] ${stamp} ${name}: ${addr ? `template <- ${addr}` : body.slice(0, 60)}`);
...
async function main() {
  ...
  if (args.includes("--telegram-test")) {
    ...
    if (who.ok && args.includes("--send")) {
      const res = await sendTelegram(cfg, "rh-pump-screener: test message", {});
...
// ---------------------------------------------------------------------------
// ON EXTENDING THIS INTO AN LP BOT
// ---------------------------------------------------------------------------
// gmgn-cli covers discovery and swaps only. Its full command surface is:
//   market  {kline, trending, trenches, signal, hot-searches, search}
//   token   {info, security, pool (READ-ONLY), holders, traders, created-tokens}
//   swap    {quote, order, cancel, multi-swap, gas-price, strategy}
//   portfolio, track, cooking, config
// There is no add/remove-liquidity or mint-position anywhere in the package, so
// the LP leg must be built directly against the AMM contracts on Robinhood
// Chain using viem or ethers against a Robinhood Chain RPC.
//
//   1. This screener tells you WHICH pool to consider. It cannot open one.
//      The launchpad_platform column shows where each token actually trades —
//      observed values include pons, pons_v2, noxa, pools_trade_instant,
//      letscash, bowfun, lunch_pair_v4 and pool_uniswap_v2/v3/v4. An LP bot
//      needs a different contract integration per venue; they are not one AMM.
//
//   2. Confirm canonical base/quote token addresses from a Robinhood Chain
//      source — gmgn-cli's currency table covers sol/bsc/base/eth only. A wrong
//      address fails silently or routes nowhere.
//
//   3. Providing liquidity to a token that just pumped +100% in an hour means
//      taking the other side of the move: as the price mean-reverts you end up
//      holding the token that fell. That is impermanent loss, and on a
//      low-liquidity memecoin it is the dominant term, not the fees. RVOL is a
//      timing input, not an entry signal — a 5m window moves on a handful of
//      trades. Backtest any threshold against `market kline` history before
//      committing capital.
