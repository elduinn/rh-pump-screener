...
// DEFAULTS
// ---------------------------------------------------------------------------
// Calibrated against the live Robinhood Chain memecoin population (median
// mcap ~$28K, median 1h volume ~$1.1K, median liquidity ~$13K), not guessed.
...
export const DEFAULTS = {
  ...
  ai: {
    ...
    promptTemplate:
      "You are an expert memecoin analyst and on-chain sleuth. Your job is to " +
...
export function validate(cfg) {
  ...
  } else if (cfg.trending.platforms.length && cfg.chain === "robinhood") {
    warnings.push(`trending.platforms is set, but gmgn-cli publishes no platform vocabulary for "robinhood" (only sol/bsc/base/eth). An unrecognized value risks silently emptying the result.`);
  }
